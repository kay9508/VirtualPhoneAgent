package com.sinyoung.exception.handler;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.google.gson.JsonParseException;
import com.sinyoung.entity.response.ErrorResponse;
import com.sinyoung.enums.ErrorCode;
...
/*
    Tip.
    @ControllerAdvice 는 @Controller 또는 @RestController 어노테이션이 붙은 모든 클래스에 적용
    @RestControllerAdvice는 @RestController 어노테이션이 붙은 클래스에만 적용

    @RestControllerAdvice는 @RequestBody 어노테이션이 포함되어 있어서 생략할 수 있다.
...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<ErrorResponse> globalExceptionHandler(MethodArgumentNotValidException ex) {
    ...
    protected ResponseEntity globalExceptionHandler(HttpClientErrorException e) {
        log.error(e.getMessage());
        final ErrorResponse response = ErrorResponse.of(ErrorCode.BAD_REQUEST_ERROR, e.getMessage());
        return new ResponseEntity<>(response, HttpStatus.valueOf(response.getStatus()));
    }

    @ExceptionHandler({ScriptRunException.class})
    protected ResponseEntity globalExceptionHandler(ScriptRunException e) {
        log.error(e.getMessage());
        final ErrorResponse response = ErrorResponse.of(ErrorCode.SCRIPT_RUN_ERROR, e.getMessage());
        return new ResponseEntity<>(response, HttpStatus.valueOf(response.getStatus()));
    }
